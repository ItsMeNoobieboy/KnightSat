...
def calibrate_gyro():
    #TODO: Set up lists, time, etc
    gyroX, gyroY, gyroZ = mag.magnetic #gauss
    
    startT = time.time()

    print("Preparing to calibrate gyrometer. Please put on flat surface.")
    maxgX = mingX = gyroX
    maxgY = mingY = gyroY
    maxgZ = mingZ = gyroZ

    while(time.time() - startT < 3):
        maxgX = max(maxgX, gyroX)
        mingX = min(mingX, gyroX)
        maxgY = max(maxgY, gyroY)
        mingY = min(mingY, gyroY)
        maxgZ = max(maxgZ, gyroZ)
        mingZ = min(mingZ, gyroZ)

    print("Calibrating...")
    offsetGX = (maxgX + mingX) / 2 
    offsetGY = (maxgY + mingY) / 2
    offsetGZ = (maxgZ + mingZ) / 2
    
    #TODO: Calculate calibration constants
    print("Calibration complete.")
    return [offsetGX,offsetGY,offsetGZ]
