...
def roll_am(accelX, accelY, accelZ):
    roll = math.atan(accelY / (accelX**2 + accelZ**2) ** 0.5)
    return roll


def pitch_am(accelX, accelY, accelZ):
    pitch = math.atan(accelX / (accelY**2 + accelZ**2) ** 0.5)
    return pitch
...
def yaw_am(accelX, accelY, accelZ, magX, magY, magZ):
    ...
    mag_x += (
        magY
        * math.sin(roll_am(accelX, accelY, accelZ))
        * math.sin(pitch_am(accelX, accelY, accelZ))
    )
    mag_x += (
        magZ
        * math.cos(roll_am(accelX, accelY, accelZ))
        * math.sin(pitch_am(accelX, accelY, accelZ))
    )
    mag_y = magY * math.cos(roll_am(accelX, accelY, accelZ)) - magZ * math.sin(
        roll_am(accelX, accelY, accelZ)
    )
